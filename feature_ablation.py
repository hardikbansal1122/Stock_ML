...
from walkforward_validation import (
    ...
    TRAIN_MONTHS, TEST_MONTHS, GAP_DAYS, STEP_MONTHS, HOLD_DAYS
)
...
EXPERIMENTS = {
    'Exp 1: Baseline': BASELINE_FEATURES,
    'Exp 2: Baseline + Market Breadth': BASELINE_FEATURES + [
        'pct_above_20dma', 'pct_above_50dma', 'adv_decl_ratio', 'new_52w_high', 'new_52w_low'
    ],
    'Exp 3: Baseline + Market Relative': BASELINE_FEATURES + [
        'stock_vs_nifty_5d', 'stock_vs_nifty_20d', 'stock_vs_nifty_60d'
    ],
    'Exp 4: Baseline + Cross-Sectional Ranking': BASELINE_FEATURES + [
        'cs_rank_20d', 'ret_5d_pctile', 'ret_60d_pctile'
    ],
    'Exp 5: Baseline + 52-Week Position': BASELINE_FEATURES + [
        '52_week_range_position'
    ]
}
...
def main():
    ...
    for exp_name, features in EXPERIMENTS.items():
        ...
        for train_start, train_end, test_start, test_end in folds:
            ...
            # Use same params and rounds as step3/walkforward
            ...
            test_res['pred_return'] = preds  # Used for simulate_portfolio
        ...
        for date, group in full_preds.groupby('Date'):
            ...
            ndcg = compute_ndcg_v2(y_true, np.sum(y_true), n) # Full NDCG
            ...
            daily_metrics.append({'P@10': p10, 'NDCG': ndcg, 'Lift': lift, 'IC': ic})
        ...
        res = {
            ...
            'ROC-AUC': f"{roc:.4f}",
            'PR-AUC': f"{pr:.4f}",
            'Precision@10': f"{mean_p10:.1%}",
            'NDCG': f"{mean_ndcg:.4f}",
            'Top-Decile Lift': f"{mean_lift:.2f}x",
            'IC': f"{mean_ic:.4f}",
            'ICIR': f"{icir:.4f}",
    ...
    md_table = df_to_markdown(df_res)
    print("\nFinal Results:")
    print(md_table)
    
    with open(REPORT_MD, 'w') as f:
        f.write("# Feature Ablation Framework Results\n\n")
        f.write(md_table)
        f.write("\n\n## Recommendation\n\n")
        
        # Logic to recommend the best
        # Parse ROC-AUC as float for sorting
        df_res['AUC_val'] = df_res['ROC-AUC'].astype(float)
        best_exp = df_res.sort_values(by='AUC_val', ascending=False).iloc[0]
        f.write(f"Based on out-of-sample ROC-AUC and generalization metrics, the optimal feature set is **{best_exp['Experiment']}**.\n")
