...
# Existing constants (preserved for backward compatibility)
HOLD_DAYS = 5
TOTAL_COST = 0.001 + 0.002  # Brokerage + Slippage
STARTING_CAPITAL = 100_000
MAX_POSITIONS = 10
POSITION_SIZE = 0.10  # Legacy fallback weight per position
...
class ModelAgnosticSimulator:
    @staticmethod
    def simulate(trades_df, prices, weights_by_day=None):
        """Simulate the portfolio.

        Args:
            trades_df: DataFrame of trades generated by PortfolioConstructor.
            prices: Dictionary mapping ticker -> price Series.
            weights_by_day: Optional dict mapping pd.Timestamp -> {ticker: weight}. When
                provided, these weights are used for allocation instead of the legacy
                POSITION_SIZE constant. Missing entries fall back to POSITION_SIZE.
        """
        if len(trades_df) == 0:
        ...
        for today in calendar_dates:
            ...
            if today in entry_buckets and len(open_positions) < MAX_POSITIONS:
                for _, t in entry_buckets[today].iterrows():
                    ...
                    if available_cash <= 0:
                        break
                    # Determine allocation weight
                    weight = None
                    if weights_by_day is not None:
                        weight = weights_by_day.get(today, {}).get(t['Ticker'])
                    effective_weight = weight if weight is not None else POSITION_SIZE
                    ...
                    locked_amount = min(available_cash, nav * effective_weight)
                    if locked_amount <= 0:
                        break
        ...
        if len(port_df) > 0:
            ...
            total_return = max_dd = cagr = sharpe = win_rate = avg_ret = 0.0
